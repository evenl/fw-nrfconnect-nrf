...
static void *at_CMT_decode(struct at_param_list *param_list, u32_t valid_params)
{
	int err;
...
int at_cmd_decoder_init(struct at_cmd_decoder_list const *decoder_list)
{
	...
	if (decoder_list != NULL) {
		at_cmd_decoders = decoder_list;
	} else {
		at_cmd_decoders = default_decoder_list;
	}
...
int at_cmd_decoder_deinit(void)
{
	...
	if (at_cmd_decoders != default_decoder_list) {
		at_cmd_decoders = default_decoder_list;
	}
