...
static void cmd_send(struct k_work *work)
{
	...
	switch (err) {
	case AT_CMD_OK:
		chars = sprintf(str, "OK\r\n");
		write_uart_string(str, chars);
		break;
	case AT_CMD_ERROR:
		chars = sprintf(str, "ERROR\r\n");
		write_uart_string(str, chars);
		break;
	case AT_CMD_CMS:
		chars = sprintf(str, "+CMS: %d\r\n", at_cmd_get_last_error());
		write_uart_string(str, chars);
		break;
	case AT_CMD_CME:
...
static void response_handler(char *response, size_t response_len)
{
	...
	if (response_len > 0) {
		...
		write_uart_string(response, response_len);
	}
}
...
static int at_host_init(struct device *arg)
{
	...
	at_cmd_set_handler(response_handler);
